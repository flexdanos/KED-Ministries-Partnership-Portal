...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [{ data: partnerApplications, error: usersError }, { data: paymentsData, error: paymentsError }] = await Promise.all([
        supabase.from('partner_applications').select('*').order('created_at', { ascending: false }),
        supabase.from('payments').select('*').gte('created_at', startDate.toISOString()).order('created_at', { ascending: false })
      ])

      if (usersError) throw usersError
      if (paymentsError) throw paymentsError

      const applicationList = partnerApplications || []
      const paymentList = paymentsData || []

      setUsers(applicationList)
      setPayments(paymentList)
      setStats(calculateDashboardStats(applicationList, paymentList))
  ...
  const calculateDashboardStats = (users: PartnerApplication[], payments: PaymentRecord[]): DashboardStats => {
    const sevenDaysAgo = new Date()
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7)
    const pendingReviews = users.filter(u => new Date(u.created_at) > sevenDaysAgo).length

    const successful = payments.filter(p => p.status === 'success')
    const failed = payments.filter(p => p.status === 'failed')
    const totalRevenue = successful.filter(p => p.amount > 0).reduce((sum, p) => sum + p.amount, 0)
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Monitor and manage the KED Ministries Portal.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Link to="/admin/users" className="group">
            <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-2 text-xs text-gray-500">Partner applications</div>
            </div>
          </Link>
          ...
          <Link to="/admin/finance" className="group">
            <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-2 text-xs text-gray-500">From {stats.totalTransactions} transactions</div>
            </div>
          </Link>
          ...
          <div className="group">
            <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-2 text-xs text-gray-500">Completed transactions</div>
            </div>
          </div>
          ...
          <div className="group">
            <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-2 text-xs text-gray-500">New this week</div>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="space-y-3">
              ...
              {payments.slice(0, 2).map((payment) => (
                <div key={payment.id} className={`flex items-center justify-between p-3 rounded-lg border ${
                  payment.status === 'success' ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'
                }`}>
                  <div>
                    <div className="font-medium text-gray-900">Partnership Contribution</div>
                    <div className="text-sm text-gray-600">{formatCurrency(payment.amount)}</div>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded ${
                    payment.status === 'success' ? 'text-emerald-700 bg-emerald-100' : 'text-amber-700 bg-amber-100'
                  }`}>
                    {payment.status}
                  </span>
                </div>
